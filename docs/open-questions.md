...
| **N19** | "When a unit is let month to month with no agreed last day, what should the system hold?" | Whether a tenancy can be genuinely open-ended, or whether every lease carries an end date. It shapes the phase-three tenancy record and decides whether renewal alerts are a date filter or something with state. | **Every lease carries an end date**, per PRD §16 — in scope even in the thin version, because forward availability cannot be answered without one. So the unit screen asks for a date to *review* the tenancy on rather than a date the tenant has agreed to leave, says so under the field, and lets it be moved at any time from "End the lease". Raised by Jeff on 2 September 2026: the requirement is a real decision, not an oversight, but a form that demands a fact nobody has needs to say what to put there. |
