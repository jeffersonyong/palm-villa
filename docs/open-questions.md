...
| **N24** | "Does the accountant need the guest's IC *inside* the accounting pack, or is the record that it was collected enough?" | The scope wording says the pack includes the IC. The pack is kept seven years and can be opened by every role that can view a booking; the IC is kept twelve months and opened only by Admin and Front Office. A copy inside the pack would outlive the original by six years and reach Security and Housekeeping. | **The pack references the IC** — when it was collected, by whom, and which record it is — and never carries the image or the filename. If the answer is "copy it in", the pack moves behind the identity permission (which Finance does not hold today) and its retention has to be decided. Raised 8 September 2026 with capability G5. See [prd.md §13](prd.md) and [architecture.md §8.2](architecture.md). |
