...
| **N14** | "All three are WhatsApp — so which one should a booking enquiry actually land on?" | Which number the public site's WhatsApp button opens. Answered in part: all three carry WhatsApp (2026-09-05), so this is now routing rather than capability. **Signal worth checking:** the client's own price list sends event enquiries to 8986733 or 8837118, not to 8959798, which is the one currently linked. | Linking the first listed (8959798). |
...
| **N22** | "If a booking is cancelled or the guest never turns up, how long do we keep the copy of their IC?" | How long identity documents are retained for a stay that never happened. Under the PDPO this is a real question rather than a tidy one: the document was collected for a purpose that then evaporated. | Twelve months from the checkout date of the stay they did not take — the same clock as a completed booking, because that is what the anchor is. Nothing shortens it. Raised 7 September 2026 with capability B10. See [prd.md §13](prd.md). |
