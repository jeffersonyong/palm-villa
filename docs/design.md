...
**A column nobody controls is the one that must not size the table.** The audit log's *Reason* is free text somebody typed: most are four words and one in twenty is a paragraph, and under auto layout that one row set the width of *When*, *Who* and *What* for every other row on the screen, so the table's proportions changed from page to page. Four columns are declared and Reason takes the remainder, and the remainder **clips to one line with a quiet "Show more" holding its own width at the end of it** — a clipped row is exactly as tall as an unclipped one, and unfolding costs its own row and nobody else's. The offer is made on the value's length, not on measuring the browser: a ref and a resize observer to answer a question a character count answers is a second render on a table that is otherwise entirely server-rendered. Cells align to the **top**, not the middle, or a timestamp floats in the middle of an unfolded row instead of marking where it starts.
