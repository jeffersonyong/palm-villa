...
**Portal forms (2026-08-27).** A form is one `booking-summary-card`, never a stack of sibling cards: sections divide with hairlines and take `caption` uppercase `mute` headers — the same header voice as data surfaces. Fields are sized to their content (a two-digit guest count gets ~170px, not a full row; only free-text like a name runs wide). The itemised price sits beside the form in a sticky `booking-summary-card` — figures in `tabular-nums`, the total at `display-xs` — carrying the screen's one primary CTA. Native selects draw their own chevron (`appearance-none` + a 16px mute chevron) so the control matches Input across browsers; the OS option picker is kept. Availability-style summary numbers render as stats: `caption` uppercase label over a `display-xs` figure — scale contrast is how a neutral surface shows hierarchy.
