...
colors:
  ...
  # (§Components — the booking calendar). A step past both its neighbours,
  # because the rows are `canvas` and `canvas-soft` is already the header
  # strip *and* a row's hover.
  group-band: "#ececec"
