...
colors:
  ...
  # owed back. Mid hues only, on the same terms as Stream.
  ledger-stay: "#2563eb"
  ledger-deposit: "#7c3aed"
...
- **Ledger colour is a fifth register, and it says *which book the money lands in*.** Two `{colors.ledger-*}` hues — blue for the stay, violet for the security deposit — on the payment verification queue, where every row is one or the other and which one decides what confirming it does to the booking (prd.md §9.1, §11). Built on Stream's terms exactly: **mid only, no tints**, identical in both themes and on every surface, a 6px dot beside its label and never a fill. `LedgerDot` is its one component. The missing tint is again the rule rather than an omission — the queue already spends `warning` on *Repriced* and `positive` on a verified tick, and those two are what mean "look at this" on a row; a tinted chip on **every** row would leave them nothing to stand out against, which is the `available`-is-neutral cost recorded below.
- **The near-miss to watch is `ledger-stay` against `stream-short-stay`** — blue ≈264° against indigo ≈277°, thirteen degrees, closer than the ≈35–45° the other registers keep. They do not meet today: the queue carries no **Type** column and the bookings list carries no ledger mark. That is the thing to look at if this ever reads wrong, and the fix is to cool `ledger-stay` towards sky rather than to give it a tint. `{colors.info}` is the system's other blue and never collides, for the reason `stream-short-stay` already rests on: `info` fills a notice panel and is never a dot.
- **The security deposit is not a fifth *tint*, and the request for one is worth recording.** A purple ground under the deposit was proposed (2026-09-05) so staff could tell it from the other gray insets on a booking screen — the identity document, the transfer slip, the accounting pack. Refused, on three grounds that hold generally. A tint that is *always* there is decoration, and a tint has to mean "look at this". Violet is already worn by an identity hue and indigo and fuchsia by two streams, so another purple would be learned against three. And the stage chip already sits on the inset, so a coloured ground would nest one tint inside another and dull the chip that needed reading. Recognition comes from **form**, as it does for every other register: the deposit's mark is the ledger's `LockKeyhole` glyph beside the micro-label *Security deposit* — `Landmark` first, until the cash-up gave banking a Record banking button wearing the same glyph, and a mark shared with an action means neither — standing directly over one three-line figure table — Held, Less charges, the line that is returned or owed — identical wherever a deposit appears (`deposit-figures.tsx`). On the Money card the mark heads the inset; on the deposit screen the section title carries it. The chip keeps the colour, because the chip answers the one question colour is for.
  ...
  **A violet *dot* was admitted later (2026-09-19), and the refusal above still stands.** What was refused was a coloured **ground** under a deposit panel; what the ledger register adds is a point beside a word in a table column, which is the form every non-status register already uses. Two of the three grounds fall away on that form: a dot is not a tint nested inside a chip, and it is not a ground that is always there under something else. **The third ground does not fall away, and is the live cost** — violet is now worn by an identity hue, by `ledger-deposit`, and indigo and fuchsia by two streams, so a reader learns four purples. What keeps them apart is form and screen: a face is a circle with two letters, a stream dot belongs to the bookings register, a ledger dot to the payments queue, and no two of them appear in the same table. If a screen is ever built that carries a stream dot and a ledger dot on one row, that is the point at which one of the two registers has to give.
