...
**Text actions.** An action worded as a sentence rather than built as a button: `body-sm` in the copy tone, underlining on hover, with the standard focus ring. The "Forgot password" register. It is for the places a control is a real action but the screen has as many bordered rectangles as it can carry — a section's `micro` title line, a field's label row — where one more reads as chrome the section has grown rather than as an offer. **The underline is the whole hover state and it appears only on hover**: at rest a permanent underline is a drawn line, and lines are spent on structure. It is not paired with a colour step, because there is no tone to step from — the ladder is ink and mute with nothing between, and an action reads as content, not as a label. Not a `Button` variant: the button owns a height and a padded box, and a link that unsets both is a variant fighting its component. Monochrome on every surface.
