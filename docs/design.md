...
**Text actions.** An action worded as a sentence rather than built as a button: `body-sm` in the copy tone, underlining on hover, with the standard focus ring. The "Forgot password" register. It is for the places a control is a real action but the screen has as many bordered rectangles as it can carry — a section's `micro` title line, a field's label row — where one more reads as chrome the section has grown rather than as an offer. **Underlined at rest**, thickening under the pointer. Hover-only was tried and is wrong: ink text with no fill, no border and no line is body copy, and a control nobody recognises until they happen to point at it is not discoverable, it just rewards the mouse. No colour carries it instead — the operations surfaces are monochrome, `copy` and `foreground` are the same ink, and the ladder's other step is mute, which reads as a label rather than an offer. The line does the work. Not a `Button` variant: the button owns a height and a padded box, and a link that unsets both is a variant fighting its component. Monochrome on every surface.
