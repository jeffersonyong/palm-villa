...
Group headers are `micro` in **`copy`** — the one place the labelling voice steps above `mute`, because here it is splitting the nav into readable blocks rather than naming a column. It stops at `copy` deliberately: `foreground` would make a non-interactive label darker than every nav item and exactly as dark as the active one, so the label would out-weigh the selection. Table headers, stat labels and form sections stay `mute`.
