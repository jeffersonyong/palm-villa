...
**[A] Existence is not content.** Anyone who may view a booking sees *that* an identity document is on file, its name and when it arrived; only opening it is gated. A guard who can see the IC was collected is being told something useful and shown nothing, and hiding the row entirely would make "did anyone take it?" unanswerable by the people whose job it is to ask. Security and Housekeeping therefore see the row and never the file, which is the principle §4 states.
