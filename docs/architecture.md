...
**One place now promises it anyway, on the client's instruction** (14 September 2026). `/booking/{token}` at the *checking* stage — after a customer says they have transferred — reads "once we verify it, we will send you an email confirmation and a QR code for entry". Recorded here rather than absorbed, because it is the one sentence in the product that describes something unbuilt: **the gate slice inherits a copy dependency**, and until it lands a guest who reaches that screen is told to expect a code nobody can issue. It was raised three times and confirmed each time; the alternative — saying nothing about how the confirmation arrives — was judged worse for a customer who has just parted with money. Nothing else, on any surface, mentions a code.
