...
- **[O] One public bucket is proposed and does not exist yet** (10 September 2026, capability F7): `site-images`, holding the public site's photographs of the units and facilities. It is named here rather than left to be discovered because it inverts every rule the four buckets above run on, and the inversion has to be deliberate. A marketing photograph is **public, cached, permanent and served to anonymous visitors**: no signed URL, no permission check on read, no access log, and — the one that would actually cause damage — **no `retain_until`**, since the retention job that protects a guest's IC would silently delete the front page. It is therefore a separate bucket with its own table, not a `kind` on `document`, and nothing in `lib/domain/document.ts` should learn about it. Writing is permission-gated like any other portal action; only reading is open. Unbuilt, and unagreed with the client.
- **F3 has since delivered both halves this note asked for** (12 September 2026): `facility` exists as a table with a stable `slug` per row that a rename never touches, and `document_retention.kind` remains a four-value check constraint, so a public image cannot acquire a retention period. F7 is unblocked and still unagreed.
- **F7 runs after F3, not beside it.** A unit type is already a table row, so an image hangs off one today — but **there is no `facility` table**, and facilities exist only as a list in the PRD and in the public site's content module. Creating one is F3's job (facility inclusion and capacity are its wording), and F7 needs a row per facility to attach an image to. Building F7 first means inventing that table and having F3 collide with it. Two things F3 should carry for F7's sake, both free at the time and a migration afterwards: a stable `slug` per facility, the way `unit_type` has one, because that is what a public page joins on; and nothing that lets `site-images` acquire a retention period — `document_retention.kind` is a four-value check constraint today, which already refuses it, and that refusal should survive F3 making retention editable.
