...
**The permission table is [A] and is in one module.** `mayOpen` / `mayAttach` in `lib/domain/document.ts` — identity opens on `document.view_identity` and is attached under `booking.amend`; a slip is `payment.verify`; a photograph is `inspection.record`; a pack is written by nobody. **Existence is separated from content**: anyone with `booking.view` sees that a document is on file and only opening it is gated. **The filename counts as content**, which review caught — an IC arrives named by whoever scanned it, and `IC_Ahmad_98765432.jpg` is the ordinary case, so a reader without `document.view_identity` is shown the kind of document rather than what it was called. That also covers the remove dialog, since `booking.amend` alone can delete an identity document without being able to open one. See PRD §13 and open-questions N23.
